...
- `adversarial-reviewer`'s own default is Opus (`model: opus` in its frontmatter) — never let it run on Sonnet or Haiku, the whole point is a second, stronger opinion. Escalate to `model: "fable"` explicitly on the `Agent` call only for the really complex/high-stakes tier (see workflow below); Opus is the floor, Fable is the ceiling.
...
3. **Adversarial review of the plan** — spawn the `adversarial-reviewer` agent on the plan before implementing; it runs on Opus by default, pass `model: "fable"` if the task is really complex, not just "complex enough to be on this list." The `advisor` tool is a fine substitute where it's available, but isn't guaranteed on every machine/setup — treat it as a bonus, not the primary mechanism.
4. **Implement.**
5. **Tests** — cover the change and its edge cases, and run the suite.
6. **Adversarial review of the code** — run `/code-review` (raise `--effort` to high/xhigh for complex/high-stakes work — that's a real, actionable lever; `/code-review` has no model parameter). Also spawn `adversarial-reviewer` on the diff (Opus by default; `model: "fable"` for the really-complex tier, same as step 3).
