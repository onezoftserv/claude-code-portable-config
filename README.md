...
**`/handoff` and `/pickup`.** Write (or read back) `.claude/handoff.md` in the current project — decisions made, current state, next steps. Auto-memory explicitly doesn't cover in-progress task state, so this fills that gap specifically for switching machines mid-task; commit the file if you want it to travel.
...
| Leave notes for picking up on another machine | `/handoff`, then `/pickup` there |
