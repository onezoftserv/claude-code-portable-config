...
allowed-tools: Read, Grep, Glob, Bash(git check-ignore*), Bash(git ls-files*), Bash(echo*), PowerShell(git check-ignore*), PowerShell(git ls-files*), PowerShell(Write-Output*)
---
...
2. Find the installed global config directory: normally `~/.claude`, but `$CLAUDE_CONFIG_DIR` overrides it if set — check with `echo $CLAUDE_CONFIG_DIR` (bash) or `Write-Output $env:CLAUDE_CONFIG_DIR` (PowerShell), since the `Read` tool can't expand env vars itself. Read `CLAUDE.md` and `settings.json` from whichever directory that resolves to — the actually-installed files, not this repo's source copies.
3. Check this project against this fixed checklist of global behaviors (don't go looking for open-ended "conflicts" — check exactly these):
   ...
   - `/code-review` (raised effort for complex/high-stakes work) plus an `adversarial-reviewer` pass (Sonnet default, `model: opus` for complex/high-stakes, `model: fable` for the really-complex tier) before calling something done.
   ...
   - **Override** — the project's `settings.json`/`settings.local.json` sets something that legitimately wins (project settings outrank user settings) — including `disableAllHooks: true` or a project `hooks` block that duplicates/replaces the guard hook, which is the likeliest real override in practice. Not a conflict — just report which value applies here.
   - **Compatible** — no conflict.
4. Separately, check for name collisions by looking for these exact paths in the project: `.claude/commands/ship.md`, `.claude/commands/check-repo-config.md`, `.claude/commands/handoff.md`, `.claude/commands/pickup.md`, `.claude/agents/adversarial-reviewer.md`, and any other `.claude/commands/`, `.claude/agents/`, or `.claude/skills/` entry whose filename matches one of ours. Project-level ones silently shadow the global ones — flag any match by name.
