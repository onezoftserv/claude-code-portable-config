...
4. Separately, check for name collisions by looking for these exact paths in the project: `.claude/commands/ship.md`, `.claude/commands/check-repo-config.md`, `.claude/agents/adversarial-reviewer.md`, and any other `.claude/commands/`, `.claude/agents/`, or `.claude/skills/` entry whose filename matches one of ours. Project-level ones silently shadow the global ones — flag any match by name.
